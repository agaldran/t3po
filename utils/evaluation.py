...
def compute_oscr_adrian(x1, x2, labels, preds):
...
def compute_oscr_decoupled(score_k, score_u, preds_k, labels):

    # score_k, score_u = np.max(probs_k, axis=1), np.max(probs_u, axis=1)

    # preds_k = np.argmax(probs_k, axis=1)
    correct = (preds_k == labels).astype(np.float32)

    k_target = np.concatenate((correct, np.zeros(len(score_u))), axis=0)
